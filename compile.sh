...
# start wait for tex file change event loop.
	...
	# if no log file is specified we use the standard log.
    if [ -z "$2" ];
    then
    	pdflatex -interaction nonstopmode $1 > $name.log
    	pdflatex -interaction nonstopmode $1 > $name.log
    else
    ...
    rm -f *.toc *.aux *.out  *.snm *.nav *.dvi *.lof $name.log 
    ...
    # we assume that when the pid!=0 evince is running
